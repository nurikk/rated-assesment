...
async def stats_action(session: db.SessionDep,
                       customer_id: str,
                       from_date: Annotated[datetime.date, Query(alias='from')]):
    statistics = await crud.get_statistics(
        session=session,
        customer_id=customer_id,
        from_date=from_date
    )
    return [
        schemas.ResourceStatisticsByDaySchema.model_validate(row) for row in statistics
    ]
